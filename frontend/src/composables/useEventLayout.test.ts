...
// toDayMinutes appelle new Date(event.start/end), qui accepte aussi bien une
// Date qu'une chaîne ISO : les fixtures ci-dessous utilisent des Date locales
// (voir commentaire plus bas) sans passer par le type CalendarEvent.
const asEvent = (event: { start: Date; end: Date }) => event as unknown as Parameters<typeof toDayMinutes>[0];
...
describe("toDayMinutes", () => {
  ...
  it("convertit un événement entièrement contenu dans le jour", () => {
    ...
    const { startMinutes, endMinutes } = toDayMinutes(asEvent(event), day);
  ...
  it("borne le début à 0 quand l'événement commence la veille", () => {
    ...
    const { startMinutes } = toDayMinutes(asEvent(event), day);
  ...
  it("borne la fin à ~1440 quand l'événement finit le lendemain", () => {
    ...
    const { endMinutes } = toDayMinutes(asEvent(event), day);
