...
export function layoutTimedEvents<T extends DayMinutes>(events: T[]): (T & LayoutBox)[] {
  const sorted = [...events].sort(
    (a, b) => a.startMinutes - b.startMinutes || b.endMinutes - a.endMinutes
  )

  const result: (T & LayoutBox)[] = []
  let cluster: T[] = []
  let clusterEnd = -Infinity

  const flush = () => {
    if (!cluster.length) return
    const columns: T[] = [] // columns[i] = dernier événement placé dans la colonne i
    const columnByEvent = new Map<T, number>()

    for (const ev of cluster) {
      let placedIn = columns.findIndex((col) => col.endMinutes <= ev.startMinutes)
      if (placedIn === -1) placedIn = columns.length
      columns[placedIn] = ev
      columnByEvent.set(ev, placedIn)
    }

    const totalColumns = columns.length
    for (const ev of cluster) {
      const col = columnByEvent.get(ev) ?? 0
      result.push({
        ...ev,
...
export function toDayMinutes(event: Pick<CalendarEvent, "start" | "end">, day: Date): DayMinutes {
  ...
  const endMinutes = Math.max((end.getTime() - dayStart.getTime()) / 60000, startMinutes + 20) // durée mini visible
